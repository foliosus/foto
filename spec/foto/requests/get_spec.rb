...
describe Foto::Requests::Get do
  let(:patient) { Foto::Patient.new }
  let(:url) { Foto::Requests::Get.new(patient).send(:build_url).to_s }
  let(:body) do
    {
      a: 'a',
      b: 'b',
  ...
  describe '#run' do
    describe 'On success' do
      it 'makes an HTTP Get request to the correct URL' do
        stub_request(:get, url)
        Foto::Requests::Get.new(patient, body).run
    ...
    describe 'On error' do
      it 'returns an instance of Foto::Requests::Response' do
        stub_request(:get, url).to_return(body: error, status: 401)
        response = Foto::Requests::Get.new(patient, body).run
        assert_requested(:get, url)
