...
describe Foto::Requests::Post do
  let(:visit) { Foto::Visit.new }
  let(:url) { Foto::Requests::Post.new(visit).send(:build_url).to_s }
  let(:body) do
    {
      a: 'a',
      b: 'b',
  ...
  describe '#run' do
    describe 'On success' do
      it 'makes an HTTP Post request to the correct URL' do
        ...
        Foto::Requests::Post.new(visit, body).run
    ...
    describe 'On error' do
      it 'returns an instance of Foto::Requests::Response' do
        ...
        response = Foto::Requests::Post.new(visit, body).run
        assert_requested(:post, url)
  ...
  describe '#url' do
    ...
    let(:uri) { Foto::Requests::Post.new(visit).send(:build_url) }
    ...
    it 'has the correct url' do
      expect(uri.to_s).to eql("#{base_api_uri}/#{visit.class.url}/json/?Api-Key=#{api_key}")
    end
  end
end
